...
## 화면
1. **모아보기** ("나의 성찰 기록") — `/`
   - 보임: 월별 달력(1순위) — 기록 있는 날에 카드 미니 이미지 표시, 오늘 강조
   - 동작: 카드 있는 날 클릭 → 상세 이동 · 월 앞뒤 이동 · 상단 '새 기록' 버튼 → 입력 화면
   - 데이터: 읽음: readings (reading_date 기준)
   - 상태: 0건이면 "아직 기록이 없어요" 안내 · 오류 메시지

2. **새 기록** ("오늘의 기록") — `/new`
   - 보임: 카드 검색란(1순위, 한글·영문 이름 검색 → 드롭다운) → 날짜·질문·해석 입력폼
   - 동작: 검색 → 카드 선택(이미지 프리뷰) → 텍스트 입력 → 저장
   - 데이터: 씀: readings
   - 상태: 저장 중 비활성화 · 저장 완료 → 모아보기 이동 · 카드 미선택 시 오류

3. **기록 상세** — `/readings/[id]`
   - 보임: 카드 이미지(좌, 1순위) + 질문·해석·리뷰(우, 2단) — 모바일은 세로 1단
...
## MVP 범위
- 포함: 모아보기(달력) · 새 기록(카드 검색) · 기록 상세(리뷰 포함)
- 제외 — 다음에: 사진 업로드 → AI 카드 인식 · 다중 카드 스프레드 기록 · 통계/분석 · 설정 화면 · 덱 선택 화면 · 카드 도감
